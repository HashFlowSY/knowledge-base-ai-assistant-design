...
export type RateLimitScope = "auth" | "user-management";
...
export function createRateLimiter(input: { store: RateLimitStore; now?: () => number }): {
  consume: (consumeInput: RateLimitConsumeInput) => Promise<RateLimitConsumeResult>;
} {
...
function parseRedisScriptResult(
  ...
  if (Array.isArray(value)) {
    ...
    const countNumber =
      typeof countValue === "number" ? countValue : Number.parseInt(String(countValue), 10);
    const resetAtNumber =
